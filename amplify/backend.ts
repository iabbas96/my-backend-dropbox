...
//To safely access auth resources
if (backend.auth && backend.auth.resources) {
  //To Get the S3 bucket from storage resource
  const { bucket } = backend.storage.resources;

  //For the Lambda to list versions
  ...
  //To Grant the authenticated role access to the bucket (if needed)
  backend.auth.resources.authenticatedUserRole.addToPrincipalPolicy(
    new PolicyStatement({
  ...
  console.warn('Auth resource not available – skipping Lambda/API setup');
}