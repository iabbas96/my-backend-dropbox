...
function FileList({ currentFolder, refreshTrigger, onFolderOpen, onShare }) {  // ← added onShare prop
  ...
  const handleShare = (path) => {
    if (onShare) onShare(path);
    else console.warn('onShare prop not provided');
  };
      ...
      {files.map(file => (
        <div key={file.path} className="file-card">
          ...
          <div className="file-actions">
            ...
            <button className="btn-share" onClick={() => handleShare(file.path)}>
              Share
            </button>
